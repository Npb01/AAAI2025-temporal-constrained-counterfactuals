...
class LTLAnalyzer(AbstractConformanceChecking):
    ...
    def run(self, jobs: int = 1, minimize_automaton: bool = False, dfa = None) -> pandas.DataFrame:
        """
        ...
        if dfa is None:
            try:
                dfa = ltl2dfa(self.process_model.parsed_formula, backend=backend2dfa)
            except:
                dfa = ltl2dfa(self.process_model.parsed_formula, backend=backend2dfa)
                print('Problem')# lydia
        else:
